...
1. 読み込まれた Skill の symlink / junction を実体パスへ解決し、その祖先から`link-targets/agents/reference-map.json`を見つけ、JSONの`repository_root`をmap所在ディレクトリから解決して instruction root を固定する。この root は共有 guide の参照専用であり、Git の work root、比較基準、終端、対象 identity、除外範囲は依頼と現在の Git 状態から別途固定する。現在の作業ディレクトリやホスト固有の絶対パスを暗黙の基準にしない。
2. instruction-root 相対の`link-targets/agents/guides/git-operations.md`を全文で読み、状態取得・権限エラー・差分範囲・範囲制御を適用する。
