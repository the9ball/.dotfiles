...
- Positive trigger: dispatch、再利用、handoff、Evidence child、role、session または epoch identity の判断が必要になる。
- Negative trigger: 短い自己完結した作業で委譲や session identity を扱わない。
- Conditional dependency: 選択した delegate model に対応する model guide だけを必要な条件で解決する。
- Failure mode: 必要な model guide や dependency contract を解決できない場合は推測で代替せず、fail-safe に停止する。
...
`link-targets/agents/AGENTS.md` の共通契約を補足し、委譲・handoff・セッション identity の詳細を所有する。
