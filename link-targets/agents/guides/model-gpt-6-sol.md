# GPT-6 Sol model guide

This is reusable model-family guidance. It does not define a role contract or a runtime tier.

## Source metadata

- Official source: <https://developers.openai.com/api/docs/guides/latest-model?model=gpt-6-sol>
- Retrieved: 2026-09-23
- Official display name: GPT-6 Sol
- Model ID: `gpt-6-sol`
- Official positioning: strong reasoning for demanding tasks, balancing intelligence and cost.
- Official reasoning levels: `none`, `low`, `medium`, `high`, `xhigh`, and `max`.
- Scope: the official GPT-6 family guidance, with Sol-specific model identity and reasoning metadata recorded here.
...
Use this profile only when the orchestrator or a role contract selects GPT-6 Sol. It supplements the normal instruction hierarchy; it never overrides system or orchestrator constraints, explicit user or task instructions, applicable `AGENTS.md` or skill instructions, safety rules, or approval boundaries. When a role contract conflicts with this guide, the role contract wins. Keep runtime tier mapping outside this document.
...
- The official API guide lists `none`, `low`, `medium`, `high`, `xhigh`, and `max` for GPT-6 Sol; select the setting intentionally.
