...
`~/.dotfiles/link-targets/agents/AGENTS.md` は全作業で常時読み込まれるため、項目を増やすほど個々の指示の遵守率が下がる。そのため、常時必要ではない詳細はここへ切り出し、AGENTS.md 側には発動条件と最小限の核だけを残す。
...
- `link-targets/agents/tools/validate-reference-map.py`を明示的な Python 3 実行で呼び出し、参照先不存在、caller 0件、循環参照を検出する。検証に失敗した状態で参照経路を移行しない。
