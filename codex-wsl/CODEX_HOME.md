...
WSL側では`CODEX_HOME=$HOME/.codex-remote`を使用します。
...
WSL側だけの制約や運用規則は`$HOME/.codex-remote/AGENTS.md`に記述します。
...
`$HOME/.codex-remote/packages/standalone/current/codex`が存在しない場合は、standalone版の導入手順を再実行します。

### ログイン状態を確認できない

`CODEX_HOME`が`$HOME/.codex-remote`を指していることと、standalone実体を明示していることを確認します。
