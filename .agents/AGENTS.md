...
- 不要なリファクタリング、cleanup、format 変更を行わない。
...
- サブエージェントまたは委譲先を dispatch するときは、選択したロール契約に対応するモデル別ガイドを補助資料として組み合わせ、対象エージェントへ渡す。GPT-6 Astra (`gpt-6-astra`) なら `guides/model-gpt-6-astra.md`、GPT-5.6 ファミリー（`gpt-5.6` alias または対応するモデルID）なら `guides/model-gpt-5.6.md` を使用する。
- モデル別ガイドは通常の repository-wide instruction として自動適用しない。root またはオーケストレーターがモデル固有の調整を必要とする場合だけ、明示的に該当ガイドを読む。
...
- 上位の指示やユーザーの明示指定を除き、メッセージ形式は、対象 Git リポジトリの明示的な規約、現在の Git user に一致する author の一貫した履歴、対象リポジトリ全体の明確な履歴、`guides/commit-message.md` のデフォルトの順に決める。履歴は直近20件から確認し、必要なら最大50件まで広げ、明確でなければ次へフォールバックする。
- merge / revert / fixup / squash / cherry-pick など、Git または既存コミットから生成・継承される特殊なメッセージは、明示的な別指示がない限り形式を保持し、このデフォルト規約で書き換えない。
- コミットの修正・取り消し・別ブランチへの移植では、通常の新規コミットに固定せず、目的に合う場合は `--fixup`、revert、cherry-pick を検討する。
- Git、formatter、lint の検証で scope 外の大量変更が生成された場合は自動的に含めず、必要なら分離または停止して確認する。
- Git、commit、検証に関するこの共通ルールは変更範囲、操作権限、検証の境界を扱い、コミットメッセージの形式や履歴規則は `guides/commit-message.md` に委ねる。

## 権限エラーと代替手段

- アクセス拒否、認証・認可不足、サンドボックス制約など、権限が原因と考えられるエラーが発生した場合は、必要な操作、対象、理由を示し、ユーザーの確認を得ること。エラーを回避する目的で、ブラウザ、GUI、別のCLI・API、別アカウント、別の保存先などへ無断で切り替えてはならない。原因を特定するための読み取り専用の確認は行ってよい。
- ただし、Git が管理領域の `index.lock`（通常は `.git/index.lock`）を作成または更新できず、`Permission denied` や `Access is denied` などの権限エラーになった場合は、同一の Git コマンドを権限昇格して再実行してよい。この規則を、その再実行に対するユーザーの事前承認として扱う。
- 現在の実行環境では通常権限で Git 管理領域へ書き込めないことがあらかじめ分かっている場合、index を書き換える Git コマンドは最初の試行から権限昇格して実行してよい。ただし、実行環境の上位ポリシーや承認機構が別途確認を要求する場合は、それに従うこと。
- `index.lock` がすでに存在する、別の Git プロセスが動作している、またはロックが残留している場合は、権限不足として扱わないこと。まず読み取り専用で原因を確認し、ロックファイルの削除やプロセスの停止は、ユーザーの明示的な承認なしに行ってはならない。
- この例外が許可するのは、権限不足で失敗した同一の Git コマンドの権限昇格だけである。ブラウザ等への切り替え、資格情報の変更、ロックファイルの削除は許可しない。
